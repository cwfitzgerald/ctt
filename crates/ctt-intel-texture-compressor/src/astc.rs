...
#[must_use]
pub fn calc_output_size(width: u32, height: u32, block_width: u32, block_height: u32) -> usize {
    // ASTC uses a fixed block size of 16 bytes (128 bits) with variable tile dimensions.
    let block_count = (width.div_ceil(block_width) * height.div_ceil(block_height)) as usize;
    block_count * 16
}
...
pub fn compress_blocks_into(settings: &EncodeSettings, surface: &RgbaSurface, blocks: &mut [u8]) {
    ...
    for yy in 0..(surface.height / settings.block_height) as u32 {
        for xx in 0..tex_width.div_ceil(program_count) {
            ...
            for i in 0..settings.fastSkipThreshold as u32 {
